...
export default async function Page({ params }) {
    const { id } = await params; // ✅ FIX: unwrapping params (Next.js 15/16)
    ...
    return (
        <>
            ...
            <div className="pt-4 px-4">
                <h3 className="lg:text-2xl text-xl text-color-black dark:text-color-primary text-center">
            ...
            <div className="flex flex-col md:gap-2 py-4 md:mx-4 mx-2">
                ...
                <a
                    ...
                    className="relative text-color-lavender text-center text-xl mb-2"
                >
                    <span className="no-underline">👉</span>{" "}
                    <span className="hover:underline">Go to Official Website</span>{" "}
                    <span className="no-underline">👈</span>
                </a>

                {/* Synopsis */}
                <div className="border-2 border-color-gold p-2 bg-color-accent rounded-lg select-none">
                    <p className="text-justify text-xl text-color-black">
                        {anime.data.synopsis}
                    </p>
                </div>
            </div>
            ...
            <div className="p-4">
                ...
                {user && (
                    <CommentInput
                        ...
                        user_email={user?.email}
                        username={user?.name}
                        anime_title={anime.data.title_english}
                    />
                )}
            </div>
